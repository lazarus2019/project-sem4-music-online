...
public class PlaylistUserController implements ServletContextAware {
	...
	/*
	 * @Autowired private PlaylistTrackService playlistTrackService;
	 */
	...
	public String add(@ModelAttribute("playlist") Playlist playlist,
			@RequestParam(value = "albumDate", required = false) String publishDate,
			@RequestParam("thumbnailAlbum") MultipartFile thumbnailAlbum,
	...
	public String edit(@ModelAttribute("playlist") Playlist playlist,
			@RequestParam("thumbnailAlbum") MultipartFile thumbnailAlbum,
			@RequestParam(value = "tracks", required = false) String[] tracks,
		...
		/*
		 * for (Track track : album.getTracks()) { album.getTracks().remove(track); }
		 */
	...
	@RequestMapping(value = {
			"reloadTracks" }, method = RequestMethod.GET, produces = MimeTypeUtils.APPLICATION_JSON_VALUE)
